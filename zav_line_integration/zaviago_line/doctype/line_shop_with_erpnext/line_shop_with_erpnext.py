...
class LineShopwithERPnext(Document):
	pass
class handleLineRequests:
	next_cursor=False
	limit = 10
	added_orders=0
	app_details = frappe.get_doc('Line Shop with ERPnext')
	...
	def save_line_order(self,o):
		...
		for product in o['orderItems']:
			...
			Item__ = frappe.db.exists({"doctype": "LINE MY SHOP Item", "line_my_shop_item_id": product['productId']})
			if( Item__ == None ):
				self.fetch_products(str(product['productId']))
	...
	def fetch_orders(self):
		...
		if( 'data' in data  ):
			...
			print(f"\n\n something is wrong2 {response} ")
		return

	def fetch_products(self,id):
		ids=""
		if( id is not None):
			ids="?ids="+str(id)
		url = "https://developers-oaplus.line.biz/myshop/v1/products"+str(ids)
		payload = {
...
def ajax_init_fetch_products():
	app_details = frappe.get_doc('Line Shop with ERPnext')
	if( app_details.enable_line_integration == 1 ):
